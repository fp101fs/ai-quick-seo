...
export function AppShell({
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100">
      ...
      {/* Desktop fixed account button — top-right corner */}
      {isSignedIn && (
        <div className="hidden lg:block fixed top-3 right-4 z-50">
          <button
          ...
          >
            {user?.picture ? (
              <img src={user.picture} alt={user?.name ?? user?.email ?? "Account"} className="w-9 h-9 rounded-full ring-2 ring-transparent hover:ring-indigo-400 transition-all shadow-sm" />
            ) : (
              <div className="w-9 h-9 rounded-full bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center ring-2 ring-transparent hover:ring-indigo-400 transition-all shadow-sm">
          ...
          {menuOpen && (
            <>
              ...
              <div className="absolute right-0 top-11 z-40 w-60 bg-white dark:bg-slate-800 rounded-xl shadow-xl border-2 border-slate-200 dark:border-slate-600 py-1 text-sm">
                ...
                <Link href="/dashboard" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <LayoutDashboard className="w-4 h-4" /> Dashboard
                </Link>
                <Link href="/usage" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <BarChart3 className="w-4 h-4" /> Usage &amp; Account
                </Link>
                <button
                ...
                >
                  ...
                  {mounted && resolvedTheme === "dark" ? "Switch to light" : "Switch to dark"}
                </button>
                {!isPro && (
                  <Link href="/pricing" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-indigo-600 dark:text-indigo-400 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors font-medium">
                    <Crown className="w-4 h-4" /> Upgrade to Pro
                  </Link>
                )}
                <div className="border-t border-slate-100 dark:border-slate-700 mt-1 pt-1">
                  <form action="/api/auth/logout" method="POST">
                    <button type="submit" className="flex w-full items-center gap-3 px-4 py-2.5 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
